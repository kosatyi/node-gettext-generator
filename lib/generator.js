...
function generateJSON( params ){
    var callback = function(){
        console.log('lang file %s/translation.json proceed',lang);
    };
    var options = {
        quiet : true
    };
    console.log(converter);
    params.locales.forEach(function(lang){
        var source = path.resolve(__dirname, [params.target,lang,'LC_MESSAGES',[params.name,'.po'].join('')].join('/') );
        var target = path.resolve(__dirname, [params.target,lang,'LC_MESSAGES','translation.json'].join('/'));
        converter.gettextToI18next(lang,source,target,options,callback);
    });
};
...
Generator.process = function(options){
    Generator.extractTranslation(options.extract).then(function(){
        return Generator(options.params)(function(){
            Generator.generateJSON(options.params);
        });
    });
};
