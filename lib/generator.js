...
function execute(command,params){
	...
	exec([command,params].join(' '), function(error,stdout){
		if (error !== null){
		    console.error('execute',command,params,error);
            defer.reject(error);
        } else {
...
function msgMerge(pofile,potfile){
    ...
    fs.open(pofile,'r',function (po){
        ...
        execute(c, o.join(' ') ).then(function(){
            ...
            console.error('msgMerge',pofile,potfile,e);
            def.reject(e);
        });
    });
    return def.promise;
...
function msgFmt(pofile,mofile){
    ...
    execute('msgfmt', opt.join(' ')).then(function(){
        ...
        console.error('msgFmt',pofile,mofile,e);
        def.reject(e);
    });
    return def;
}
...
function findFiles(source){
	...
	deferred.map(source,function(folder){
		...
		filewalker( folder ).on('file',function(p){
		    ...
		    console.error('findFiles',folder,e);
			def.reject(e);
		}).walk();
		return def.promise;
	}).then(function(){
		result.resolve(files.sort());
	});
...
function generateJS(data,options){
    options = extend({namespace:'i18n'},options||{});
    data.forEach(function(item){
        var source  = fs.readFileSync(item.file);
        var target  = path.resolve([item.path,'translation.js'].join('/'));
        var content = util.format('(function(n,k,v){this[n]=this[n]||{};this[n][k]=v})(%j,%j,%s);',
            options.namespace,
            item.lang,
            source.toString()
        );
